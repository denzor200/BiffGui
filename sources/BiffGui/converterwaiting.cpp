...
{
    getProcess()->start("openssl\\bin\\openssl genrsa -out client.key 2048");
}
...
void OpensslWaiting_Req::StartProcess(
        ...
        const QString& CN)
{
    getProcess()->start(QString() + "openssl\\bin\\openssl req -new -key client.key -out \"" + FileName + "\"  -config openssl\\bin\\cnf\\openssl.cnf");
